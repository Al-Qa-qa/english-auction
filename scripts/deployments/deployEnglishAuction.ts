...
import { EnglishAuction, EnglishAuction__factory } from "../../typechain-types";
 ...
 */
async function deployEnglishAuction(chainId: number, nftAddress: string) {
  ...
  log(`EnglishAuction deployed to: ${englishAuction.address}`);
  log("", "separator");

  if (!developmentChains.includes(network.name)) {
    // Verify Contract if it isnt in a development chain
    log("Verifying Contract", "title");
    await englishAuction.deployTransaction.wait(VERIFICATION_BLOCK_CONFIRMATIONS);
    await verify(englishAuction.address, []);
  ...
  (oldContracts[network.name].EnglishAuction = englishAuction.address),
